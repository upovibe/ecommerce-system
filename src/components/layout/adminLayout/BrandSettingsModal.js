...
class BrandSettingsModal extends HTMLElement {
  constructor() {
    super();
  ...
  onConfirm = async () => {
    try {
      ...
      const createRes = await api.withToken(token).post("/brands", payload);
      ...
      if (newId && newImage) {
        ...
        await api.withToken(token).post(`/brands/${newId}/upload-image`, formData, {
          headers: {
  ...
  render() {
    this.innerHTML = `
      <ui-modal ${this.hasAttribute("open") ? "open" : ""} position="right" size="md" close-button="true">
        <div slot="title">Add Brand</div>
        <form class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Brand Name</label>
            <ui-input data-field="name" placeholder="e.g. Nike" class="w-full"></ui-input>
          </div>
          <div>
...
customElements.define("brand-settings-modal", BrandSettingsModal);
